...
```bash
# Install from .skillet/config/sources.json (bootstraps defaults on first run); prompts once for agent targets
skillet install
...
- **Native skill directories**: for each **enabled** agent target, Skillet copies each installed skill to that agent’s standard skills path — see [Where skills are discovered](#where-skills-are-discovered) below. Skillet does **not** generate `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, or a Skillet Cursor rules file. On `install` / `sync` / `add`, it deletes **known** legacy Skillet outputs if they still exist (for example `.cursor/rules/skillet.mdc` and `.github/copilot-instructions.md`).
...
| **Native skill mirrors** | See [Where skills are discovered](#where-skills-are-discovered) | One folder per skill (`<name>/SKILL.md`) under each *enabled* agent’s path. Stale skills are pruned when you `skillet remove`, delete a skill from `sources.json` and run `skillet sync`, or disable an agent. Regenerated by `install` / `sync` / `add`. |
